...
def main():
    # 配置路径
    config_path = './dataset/process_text_add_tom'
    sector_results = []
    for sector in os.listdir(config_path):
        test_file = os.path.join(config_path, sector, 'test_multimodal.csv')
        model_path = os.path.join(config_path, sector, 'best_multi_stock_model.pkl')
        scaler_path = os.path.join(config_path, sector, 'multi_stock_scaler.pkl')
        feature_columns_path = os.path.join(config_path, sector, 'multi_stock_feature_cols.pkl')
        selected_indices_path = os.path.join(config_path, sector, 'multi_stock_features.pkl')
        ...
        print("\n" + "="*60)
        print(f"测试完成 {sector}板块,准确率: {accuracy:.4f}")
        print("="*60)

    sector_df = pd.DataFrame(sector_results)
    sector_df.to_csv(os.path.join(config_path,'test_result.csv'))
