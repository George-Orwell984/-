...
class PyramidConv1D(nn.Module):
    ...
    def forward(self, x):  # x: (batch, seq_len, feature_dim)
        ...
        outs = [conv(x) for conv in self.convs]  # 多尺度
        out = torch.cat(outs, dim=1)  # (batch, out_channels*len(kernel_sizes), seq_len)
        out = out.transpose(1, 2)  # (batch, seq_len, out_channels*len(kernel_sizes))
        return out
...
class PyramidConvLSTMAttentionModel(nn.Module):
    def __init__(self, input_size, conv_out=32, kernel_sizes=[3,7,15], lstm_hidden=64, lstm_layers=1, dropout=0.3):
        super().__init__()
        self.pyramid = PyramidConv1D(input_size, conv_out, kernel_sizes)
        self.lstm = nn.LSTM(
            input_size=conv_out * len(kernel_sizes),
    ...
    def forward(self, x):  # x: (batch, seq_len, feature_dim)
        feat = self.pyramid(x)  # (batch, seq_len, conv_out*len(kernel_sizes))
        lstm_out, _ = self.lstm(feat)  # (batch, seq_len, lstm_hidden*2)
...
def train_sector_model(sector_name):
    ...
    lstm_layers = 1
    dropout = 0.3
    model = PyramidConvLSTMAttentionModel(
    ...
    print(f"模型配置: PyramidConv+LSTM+Attention, conv_out={conv_out}, kernel_sizes={kernel_sizes}, lstm_hidden={lstm_hidden}, lstm_layers={lstm_layers}, dropout={dropout}")
    print(f"模型参数数量: {sum(p.numel() for p in model.parameters())}")
    
    # 8. 训练模型
    def train_model_conv(model, train_loader, val_loader, num_epochs=50, learning_rate=0.001):
        print("开始训练模型...")
        criterion = nn.BCELoss()
        optimizer = optim.Adam(model.parameters(), lr=learning_rate, weight_decay=1e-4)
        scheduler = optim.lr_scheduler.ReduceLROnPlateau(
        ...
        model.load_state_dict(torch.load('best_model.pth'))
        return model, train_losses, val_losses, val_aucs
    model, train_losses, val_losses, val_aucs = train_model_conv(
    ...
    plt.plot(fpr, tpr, label=f'PyramidConv+LSTM+Attention (AUC={test_auc:.3f})')
    ...
    plt_save_path = os.path.join(sector_path, f'{sector_name}_pyramid_conv_lstm_attn_results.png')
    ...
    # 保存到板块特定目录
    results_save_path = os.path.join(sector_path, f'{sector_name}_pyramid_conv_lstm_attn_predictions.csv')
    results_df.to_csv(results_save_path, index=False)
    
    # 保存模型和预处理器
    model_save_path = os.path.join(sector_path, f'{sector_name}_pyramid_conv_lstm_attn_model.pth')
    scaler_save_path = os.path.join(sector_path, f'{sector_name}_pyramid_conv_lstm_attn_scaler.pkl')
    features_save_path = os.path.join(sector_path, f'{sector_name}_pyramid_conv_lstm_attn_features.pkl')
    ...
    print("结果已保存:")
    print(f"- {sector_name}_pyramid_conv_lstm_attn_predictions.csv: 预测结果")
    print(f"- {sector_name}_pyramid_conv_lstm_attn_model.pth: 模型权重")
    print(f"- {sector_name}_pyramid_conv_lstm_attn_scaler.pkl: 特征标准化器")
    print(f"- {sector_name}_pyramid_conv_lstm_attn_features.pkl: 特征列名")
    print(f"- {sector_name}_pyramid_conv_lstm_attn_results.png: 可视化图表")
